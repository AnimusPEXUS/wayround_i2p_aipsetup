...
def get_python_packages(python_pkg_dir):
    ...
    for i in weak_names:
        if not i in pkg_names_lst:
            print(
                """\
------------------------------------------------------------------------------
PLEASE NOTE: [{}] automatic downloading are
             currently disabled. please download theyr python distributions
             manually and place them into '{}' dir, after what restart
             `aipsetup sys-replica make-flash' cmd.
             Sorry for inconvinience, this will be fixed some time in the
             future.
------------------------------------------------------------------------------
""".format('|'.join(weak_names), os.path.basename(python_pkg_dir))
                )
            break
    return
...
def install_python_packages(output_dir, python_pkg_dir):
    ...
    target_mont_dir = wayround_i2p.utils.path.join(
        output_dir,
        'py_pack'
        )
    ...
    if ret == 0:
        # TODO: following script MUST return error codes, not silence them!
        p = subprocess.Popen(
            [
                'chroot', output_dir,
                '/bin/bash',
...
def install_packages(packages_dir, output_dir):
    ...
    for i in range(len(aips) - 1, -1, -1):
        val = aips[i]
        val_j = wayround_i2p.utils.path.join(packages_dir, val)
...
def make_initrd_init(output_dir, boot_part_uuid):
    dirname = wayround_i2p.utils.path.join(output_dir)
    filename = wayround_i2p.utils.path.join(dirname, 'init.sh')
...
echo
echo
echo 'WELCOME TO LAILALO SETUP DRIVE IMAGE'
echo
echo
echo
...
def make_root_init_merged(output_dir, boot_part_uuid):
    dirname = wayround_i2p.utils.path.join(output_dir)
    filename = wayround_i2p.utils.path.join(dirname, 'init_merged.sh')
...
def clean_linux_source(output_dir):
    ...
    dirname = wayround_i2p.utils.path.join(
        output_dir,
        'multihost',
        '_primary',
        'src',
...
def make_boot_dir(boot_d, root_d):
    ...
    for i in lst:
        sj = wayround_i2p.utils.path.join(src_dir, i)
        bj = wayround_i2p.utils.path.join(boot_d, i)
...
def make_extlinux_conf(
    ...
    dirname = wayround_i2p.utils.path.join(output_dir, 'extlinux')
    filename = wayround_i2p.utils.path.join(dirname, 'extlinux.conf')
...
def smart_redo_aips_dirs(
    ...
    for i in [
        ...
        if wayround_i2p.utils.checksum.is_dir_changed(
                i[0],
                i[0] + '.sha512',
                verbose=True
                ):
            if os.path.exists(i[1]):
                wayround_i2p.utils.file.remove_if_exists(i[1])
    return
...
def create_flashdrive_image(
    ...
    python_pkg_dir = wayround_i2p.utils.path.join(working_dir, 'py_packs')

    boot_d = wayround_i2p.utils.path.join(working_dir, 'boot')
    boot_tar = wayround_i2p.utils.path.join(working_dir, 'boot.tar')

    initrd_d = wayround_i2p.utils.path.join(working_dir, 'initrd')
    initrd_f = wayround_i2p.utils.path.join(working_dir, 'initrd_aips')

    root_d = wayround_i2p.utils.path.join(working_dir, 'root')
    root_f = wayround_i2p.utils.path.join(working_dir, 'root_aips')

    snap_d = wayround_i2p.utils.path.join(working_dir, 'snap')

    initrd_d_init_sh = wayround_i2p.utils.path.join(initrd_d, 'init.sh')
    root_d_init_sh = wayround_i2p.utils.path.join(root_d, 'init.sh')

    mnt_d = wayround_i2p.utils.path.join(working_dir, 'mnt')

    squash_params = ['-comp', 'xz', '-no-exports', '-all-root']

    initrd_squash = wayround_i2p.utils.path.join(working_dir, 'initrd.squash')
    initrd_squash_comp = initrd_squash + initrd_final_compressor_ext
    root_squash = wayround_i2p.utils.path.join(working_dir, 'root.squash')

    snap_squash = wayround_i2p.utils.path.join(working_dir, 'snap.squash')

    dst_initrd_squash_comp = wayround_i2p.utils.path.join(
        boot_d,
        'initrd.squash' + initrd_final_compressor_ext
        )
    dst_root_squash = wayround_i2p.utils.path.join(boot_d, 'root.squash')
    dst_snap_squash = wayround_i2p.utils.path.join(boot_d, 'snap.squash')

    loop_dev_name = 'loop0'
    loop_dev_path = wayround_i2p.utils.path.join('/dev', loop_dev_name)
    loop_dev_part1_name = loop_dev_path + 'p1'

    boot_img = wayround_i2p.utils.path.join(working_dir, 'boot.bin')

    mnt_extlinux_path = wayround_i2p.utils.path.join(mnt_d, "extlinux")
    ...
    os.makedirs(
        wayround_i2p.utils.path.join(initrd_d, 'root_new'),
        exist_ok=True
        )

    # os.makedirs(
    #    wayround_i2p.utils.path.join(initrd_d, 'root_new_ro'),
    #    exist_ok=True
    #    )

    os.makedirs(
        wayround_i2p.utils.path.join(root_d, 'root_old'),
        exist_ok=True
        )
    os.makedirs(
        wayround_i2p.utils.path.join(root_d, 'snap'),
        exist_ok=True
        )

    os.makedirs(
        wayround_i2p.utils.path.join(root_d, 'overlay'),
        exist_ok=True
        )

    os.makedirs(
        wayround_i2p.utils.path.join(root_d, 'overlay_merged'),
        exist_ok=True
        )
...
""".format(
    ...
    res = wayround_i2p.utils.file.copytree(

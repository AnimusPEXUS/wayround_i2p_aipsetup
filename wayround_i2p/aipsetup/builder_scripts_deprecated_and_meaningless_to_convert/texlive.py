...
def main(buildingsite, action=None):
    ...
    if not isinstance(r, tuple):
        ...
        install_tl_dir = wayround_i2p.utils.path.join(buildingsite, 'install-tl')
        ...
        dst_tex_live_dir = wayround_i2p.utils.path.join(dst_dir, 'usr', 'lib', 'texlive')

        dst_tex_live_bin = wayround_i2p.utils.path.join(dst_tex_live_dir, 'bin')
...
#                    '--target=' + pkg_info['constitution']['target']
        ...
        if 'distribute' in actions and ret == 0:
            ...
            p_dir = wayround_i2p.utils.path.join(dst_dir, 'etc', 'profile.d', 'SET')

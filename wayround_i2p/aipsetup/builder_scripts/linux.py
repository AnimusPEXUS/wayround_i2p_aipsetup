...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ...
        ret = dict()
        ret['src_arch_dir'] = wayround_i2p.utils.path.join(
            self.get_src_dir(), 'arch'
            )
        ret['dst_boot_dir'] = wayround_i2p.utils.path.join(
            self.get_dst_dir(), 'boot'
            )
        ret['dst_man_dir'] = wayround_i2p.utils.path.join(
...
""")
    ...
    def builder_action_distr_modules(self, called_as, log):
        ...
        if ret == 0:
            ...
            modules_dir = wayround_i2p.utils.path.join(
            ...
            if len(files) != 1:
                ...
                for i in ['build', 'source']:
                    ...
                    new_link = wayround_i2p.utils.path.join(
                        modules_dir,
                        i
                        )

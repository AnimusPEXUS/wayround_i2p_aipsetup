...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ...
        return {
            'src_dir_p_sep': wayround_i2p.utils.path.join(
                self.get_src_dir(),
    ...
    def builder_action_configure(self, called_as, log):
        ...
        t_conf = wayround_i2p.utils.path.join(src_dir_p_sep, '.config')

        shutil.copyfile(
            wayround_i2p.utils.path.join(src_dir_p_sep, 'defconfig'),
            t_conf
            )

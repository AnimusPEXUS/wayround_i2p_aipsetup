        ...
        if tar is None:
            ...
            if p_r != 0:
                ...
                if not tar_dir in os.listdir(self.get_src_dir()):
                    ...
                    tar_dir = wayround_i2p.utils.path.join(
                        self.get_src_dir(),
                        tar_dir
                        )
                    lsof_file = wayround_i2p.utils.path.join(
                        tar_dir,
                        'lsof'
                        )
                    lsof_man_file = wayround_i2p.utils.path.join(
                        tar_dir,
                        'lsof.8'
                        )
    ...
    def builder_action_distribute(self, called_as, log):
        ...
        if not os.path.isfile(lsof_file):
            ...
            if not os.path.isfile(lsof_man_file):
                ...
                dst_man_dir = wayround_i2p.utils.path.join(
                    self.calculate_dst_install_prefix(),
                    'share',
                    'man',
                    'man8'
                ...
                shutil.copy(
                    lsof_man_file,
                    wayround_i2p.utils.path.join(dst_man_dir, 'lsof.8')
                    )

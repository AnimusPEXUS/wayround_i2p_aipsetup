...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def builder_action_configure(self, called_as, log):
        ...
        makefile = wayround_i2p.utils.path.join(
            self.get_src_dir(),
    ...
    def builder_action_distribute(self, called_as, log):
        ...
        dist_dir = wayround_i2p.utils.path.join(
            self.get_src_dir(),
            'dist'
            )

        OBJ_dir = glob.glob(wayround_i2p.utils.path.join(dist_dir, '*.OBJ'))
        ...
        if ret == 0:
            ...
            OBJ_dir = wayround_i2p.utils.path.join(
                dist_dir,
                os.path.basename(OBJ_dir)
                )

            OBJ_dir_bin = wayround_i2p.utils.path.join(OBJ_dir, 'bin')
            OBJ_dir_lib = wayround_i2p.utils.path.join(
                OBJ_dir,
                'lib'
                )
            OBJ_dir_include = wayround_i2p.utils.path.join(OBJ_dir, 'include')

            OBJ_dir_rmarch = wayround_i2p.utils.path.join(
                OBJ_dir,
                'multihost'
                )

            OBJ_dir_march = wayround_i2p.utils.path.join(
                OBJ_dir,
                self.calculate_install_prefix()
                )

            OBJ_dir_ma_bin = wayround_i2p.utils.path.join(
                OBJ_dir_march,
                'bin'
                )
            OBJ_dir_ma_lib = wayround_i2p.utils.path.join(
                OBJ_dir_march,
                'lib'
                )
            OBJ_dir_ma_lib_pkgconfig = wayround_i2p.utils.path.join(
                OBJ_dir_ma_lib,
                'pkgconfig'
                )
            OBJ_dir_ma_include = wayround_i2p.utils.path.join(
                OBJ_dir_march,
                'include'
                )

            OBJ_dir_ma_include_nss = wayround_i2p.utils.path.join(
                OBJ_dir_ma_include,
                'nss'
                )
        ...
        if ret == 0:
            ...
            for i in files:
                log.info("    {}".format(i))
                if wayround_i2p.utils.file.convert_symlink_to_file(
                        wayround_i2p.utils.path.join(OBJ_dir_bin, i)
                        ) != 0:
                    errors += 1
                    log.info("        error")
                else:
        ...
        if ret == 0:
            ...
            for i in files:
                log.info("    {}".format(i))
                if wayround_i2p.utils.file.convert_symlink_to_file(
                        wayround_i2p.utils.path.join(OBJ_dir_lib, i)
                        ) != 0:
                    errors += 1
                    log.info("        error")
                else:
        ...
        if False:
            ...
            for i in files:
                if not i in ['certutil', 'nss-config', 'pk12util']:
                    os.unlink(
                        wayround_i2p.utils.path.join(OBJ_dir, 'bin', i)
                        )
            '''
            '''
            if wayround_i2p.utils.file.dereference_files_in_dir(
                    OBJ_dir
                    ) != 0:
                log.error(
        ...
        if ret == 0:
            ...
            for i in ['private', 'public']:
                wayround_i2p.utils.file.files_by_mask_copy_to_dir(
                    wayround_i2p.utils.path.join(
                        dist_dir,
                        i,
                        'nss'
...
""".format(
            ...
            f = open(
                wayround_i2p.utils.path.join(OBJ_dir_ma_bin, 'nss-config'),
                'w'
                )

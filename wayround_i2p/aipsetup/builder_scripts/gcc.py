...
import wayround_i2p.aipsetup.builder_scripts.binutils
...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ...
        ret = dict()
        ret['cc_file'] = wayround_i2p.utils.path.join(
            self.calculate_dst_install_prefix(), 'bin', 'cc'
            )
        ret['libcpp_file'] = wayround_i2p.utils.path.join(
    ...
    def builder_action_configure_define_opts(self, called_as, log):
        ...
        if self.get_is_crossbuilder():
            ...
            ret = [
                '--prefix=' + prefix,
                '--mandir=' +
                wayround_i2p.utils.path.join(prefix, 'share', 'man'),
                '--sysconfdir=/etc',
                '--localstatedir=/var',
                '--enable-shared',
                '--disable-gold',
    ...
    def builder_action_build_define_environment(self, called_as, log):
        ...
        if self.get_arch_from_pkgi().startswith('x86_64'):
            ...
            for i in ['lib', 'lib64', 'lib32', 'libx32']:
                ...
                joined = wayround_i2p.utils.path.join(

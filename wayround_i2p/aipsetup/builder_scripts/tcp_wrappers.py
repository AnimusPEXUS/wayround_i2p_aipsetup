...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def builder_action_distrinute(self, called_as, log):
        ...
        for i in [
            ...
            pp = wayround_i2p.utils.path.join(self.get_dst_dir(), i)
            if not os.path.isdir(pp):
                os.makedirs(pp)

        # shared

        shared_dir = wayround_i2p.utils.path.join(self.get_src_dir(), 'shared')
        ...
        for i in shared:
            if i.startswith('libwrap'):
                p = subprocess.Popen(
                    [
                        'cp',
                        wayround_i2p.utils.path.join(shared_dir, i),
                        wayround_i2p.utils.path.join(
                            self.get_dst_dir(),
                            'usr/lib'
                        )
        ...
        for i in os.listdir(self.get_src_dir()):
            if i.endswith('.a'):
                p = subprocess.Popen(
                    [
                        'cp',
                        wayround_i2p.utils.path.join(self.get_src_dir(), i),
                        wayround_i2p.utils.path.join(
                            self.get_dst_dir(),
                            'usr/lib'
                        )
        ...
        for i in ['safe_finger', 'tcpd', 'tcpdchk', 'tcpdmatch', 'try-from']:
            p = subprocess.Popen(
                ['cp',
                 wayround_i2p.utils.path.join(self.get_src_dir(), i),
                 wayround_i2p.utils.path.join(self.get_dst_dir(), 'usr/bin')
                 ]
            )
            if p.wait() != 0:
                ret = 6
        ...
        for i in range(10):
            dd = wayround_i2p.utils.path.join(
                self.get_dst_dir(),
                'usr',
                'share',
                'man',
            ...
            for j in b:
                p = subprocess.Popen(
                    ['cp',
                     wayround_i2p.utils.path.join(j),
                     wayround_i2p.utils.path.join(dd, os.path.basename(j))
                     ]
                )
                if p.wait() != 0:
                    ret = 7

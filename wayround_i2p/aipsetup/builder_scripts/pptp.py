...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ...
        ret = {
            'PPPD': wayround_i2p.utils.file.which(
                'pppd',
                self.calculate_install_prefix()
                ),
            'BINDIR': wayround_i2p.utils.path.join(dst_prefix, 'sbin'),
            'MANDIR': wayround_i2p.utils.path.join(
                dst_prefix,
                'share',
                'man',
                'man8'
                ),
            #'PPPDIR': wayround_i2p.utils.path.join(dst_prefix, 'etc', 'ppp')
            }
        return ret

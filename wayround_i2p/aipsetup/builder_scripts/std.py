...
class Builder:
    ...
    def get_multihost_dir(self):
        return wayround_i2p.utils.path.join(
            os.path.sep,
            wayround_i2p.aipsetup.build.ROOT_MULTIHOST_DIRNAME
            )
    ...
    def get_host_multiarch_dir(self):
        return wayround_i2p.utils.path.join(
            self.get_host_dir(),
            wayround_i2p.aipsetup.build.MULTIHOST_MULTIARCH_DIRNAME
            )
    ...
    def get_host_crossbuilders_dir(self):
        return wayround_i2p.utils.path.join(
            self.get_host_dir(),
            wayround_i2p.aipsetup.build.MULTIHOST_CROSSBULDERS_DIRNAME
            )
    ...
    def builder_action_patch(self, called_as, log):
        ...
        for i in series:
            p_file = wayround_i2p.utils.path.join(
                self.get_patches_dir(),
                i
            )
    ...
    def builder_action_configure_define_opts(self, called_as, log):
        ...
        ret += [
            ...
            # '--libdir=' + wayround_i2p.utils.path.join(
            ...
            '--sysconfdir=/etc',
            # '--sysconfdir=' + wayround_i2p.utils.path.join(
            #     self.get_host_arch_dir(),
            #     'etc'
            #     ),
            '--localstatedir=/var',

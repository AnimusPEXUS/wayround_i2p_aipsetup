...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ...
        ret = {
            'plugin_dir': wayround_i2p.utils.path.join(
                self.calculate_install_prefix(),
                '/opt/unbit/uwsgi/plugins'
                )
            }
        ret['dst_plugin_dir'] = wayround_i2p.utils.path.join(
            self.get_dst_dir(),
            ret['plugin_dir']
            )
    ...
    def builder_action_configure(self, called_as, log):
        ...
        cfg_filename = wayround_i2p.utils.path.join(
            self.get_src_dir(),
            'buildconf',
            'wayround_i2p.ini'
            )
...
""".format(
    ...
    def builder_action_build(self, called_as, log):
        ...
        '''
        apr_ldflags = wayround_i2p.utils.pkgconfig.pkgconfig(
            'apr-1',
            '--libs'
            ).split()

        apr_util_ldflags = wayround_i2p.utils.pkgconfig.pkgconfig(
            'apr-util-1',
            '--libs'
            ).split()
        ...
        #env = wayround_i2p.utils.osutils.env_vars_edit(env)
    ...
    def builder_action_distribute(self, called_as, log):
        ...
        dst_bin_dir = wayround_i2p.utils.path.join(
        ...
        for i in plugins:
            i_dst = wayround_i2p.utils.path.join(
                self.custom_data['dst_plugin_dir'],
                os.path.basename(i)
                )

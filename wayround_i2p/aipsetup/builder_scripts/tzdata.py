...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ret = dict()
        ret['makefile'] = wayround_i2p.utils.path.join(
            self.get_src_dir(), 'Makefile'
            )
        ret['zoneinfo'] = wayround_i2p.utils.path.join(
            self.calculate_dst_install_prefix(),
            'share',
            'zoneinfo'
            )
        ret['zoneinfop'] = wayround_i2p.utils.path.join(
            ret['zoneinfo'],
            'posix'
            )
        ret['zoneinfor'] = wayround_i2p.utils.path.join(
            ret['zoneinfo'],
            'right'
            )
        return ret

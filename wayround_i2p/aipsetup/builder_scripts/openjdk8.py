...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def builder_action_configure_define_opts(self, called_as, log):
        ...
        ret += [
            ...
            # NOTE: next line may be needed if you have no working openjpd
            #       installed and so need to use Oracle's binary distribution
    ...
    def builder_action_after_distribute(self, called_as, log):
        ...
        java_dir = wayround_i2p.utils.path.join(
            self.calculate_install_prefix(),
            'opt',
            'java'
            )

        dst_java_dir = wayround_i2p.utils.path.join(
            self.get_dst_dir(),
            java_dir
            )

        etc_dir = wayround_i2p.utils.path.join(
            self.get_dst_dir(),
            'etc',
            'profile.d',
            'SET'
        ...
        java009 = wayround_i2p.utils.path.join(
            etc_dir,
            '009.java.{}.{}.sh'.format(
        ...
        if ret == 0:
            try:
                for i in [
                        wayround_i2p.utils.path.join(dst_java_dir, 'jre'),
                        wayround_i2p.utils.path.join(dst_java_dir, 'jdk'),
                        wayround_i2p.utils.path.join(dst_java_dir, 'java')
                        ]:

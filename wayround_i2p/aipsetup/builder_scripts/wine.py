...
class Builder_wow64(wayround_i2p.aipsetup.builder_scripts.std.Builder):

    def define_custom_data(self):

        self.separate_build_dir = wayround_i2p.utils.path.join(
            self.get_src_dir(),
            'wine64'
            )
    ...
    def define_actions(self):
        ...
        t = wayround_i2p.aipsetup.builder_scripts.std.Builder.define_actions(
            self
            )
...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ...
        if self.get_arch_from_pkgi().startswith('x86_64'):
            ...
            '''
            if wayround_i2p.utils.file.which(
...
""")
    ...
    def builder_action_configure_define_opts(self, called_as, log):
        ...
        if self.custom_data['Builder_wow64'] is not None:
            ...
            ret += [
                '--with-wine64={}'.format(
                    wayround_i2p.utils.path.join(self.get_src_dir(), 'wine64')
                    )
                ]

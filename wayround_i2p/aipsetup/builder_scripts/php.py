...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ...
        ret['dst_etc_dir'] = wayround_i2p.utils.path.join(
            self.get_dst_dir(),
            ret['etc_dir']
            )
    ...
    def builder_action_configure_define_opts(self, called_as, log):
        ...
        curses_cflags = wayround_i2p.utils.pkgconfig.pkgconfig(
            'ncurses', '--cflags'
            )

        curses_libs = wayround_i2p.utils.pkgconfig.pkgconfig(
            'ncurses', '--libs'
            )
    ...
    def builder_action_after_distribute(self, called_as, log):
        ...
        for i in lst:
            if i.startswith('.'):
                wayround_i2p.utils.file.remove_if_exists(
                    wayround_i2p.utils.path.join(
                        self.get_dst_dir(),
                        i
                        )

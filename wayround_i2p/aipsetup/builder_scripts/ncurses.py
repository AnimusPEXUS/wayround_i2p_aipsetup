...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ret = {}
        ret['pth_dir'] = \
            wayround_i2p.aipsetup.build.getDIR_PATCHES(
        ...
        if len(pth_files) == 0:
            ...
            if rolling:
                ...
                if p.wait() != 0:
                    ...
                    p = subprocess.Popen(
                        ['bash',
                         wayround_i2p.utils.path.join(pth_dir, rolling)],
        ...
        if ret == 0:
            ...
            for i in patches:
                ...
                if p.wait() != 0:
                    ...
                    p = subprocess.Popen(
                        ['patch', '-p1', '-i',
                         wayround_i2p.utils.path.join(pth_dir, i)],
                        cwd=self.get_src_dir(),
                        stdout=log.stdout,
                        stderr=log.stderr
                        )
    ...
    def builder_action_links(self, called_as, log):
        ...
        for s in [
            ...
            files = glob.glob(
                wayround_i2p.utils.path.join(dst_lib_dir, s[0])
                )
            ...
            for i in files:
                ...
                rrr = wayround_i2p.utils.path.join(dst_lib_dir, l_name)
        ...
        for i in links:
            ...
            flp = wayround_i2p.utils.path.join(dst_lib_dir, i)

            if os.path.islink(flp):

                rflp = wayround_i2p.utils.path.realpath(flp)
    ...
    def builder_action_pc(self, called_as, log):
        ...
        for s in [
            ...
            files = glob.glob(
                wayround_i2p.utils.path.join(dst_pc_lib_dir, s[0])
                )
            ...
            for i in files:
                ...
                rrr = wayround_i2p.utils.path.join(dst_pc_lib_dir, l_name)

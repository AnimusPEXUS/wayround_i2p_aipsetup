...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def builder_action_configure(self, called_as, log):
        new_make_filename = wayround_i2p.utils.path.join(
            self.get_src_dir(),
            'pdftk',
            'Makefile.Lailalo'
            )
...
""".format(
    ...
    def builder_action_build(self, called_as, log):
        p = subprocess.Popen(
            ...
            cwd=wayround_i2p.utils.path.join(self.get_src_dir(), 'pdftk'),
            stdout=log.stdout,
            stderr=log.stderr
            )
        ret = p.wait()
    ...
    def builder_action_distribute(self, called_as, log):
        ...
        sbin = wayround_i2p.utils.path.join(
            self.get_src_dir(),
            'pdftk',
            'pdftk'
            )
        bin_dir = wayround_i2p.utils.path.join(
        ...
        if not os.path.isdir(bin_dir):
            ...
            shutil.copy(
                sbin,
                wayround_i2p.utils.path.join(bin_dir, 'pdftk')
                )

        sman = wayround_i2p.utils.path.join(self.get_src_dir(), 'pdftk.1')
        man = wayround_i2p.utils.path.join(
        ...
        if not os.path.isdir(man):
            ...
            shutil.copy(sman, wayround_i2p.utils.path.join(man, 'pdftk.1'))
        return ret

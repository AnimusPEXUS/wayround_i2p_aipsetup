...
import wayround_i2p.aipsetup.builder_scripts.qemu
...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def define_custom_data(self):
        ret = {
            'qemu_builder': wayround_i2p.aipsetup.builder_scripts.qemu.Builder(
                self.control
                )
            }
        return ret
    ...
    def builder_action_configure_define_opts(self, called_as, log):
        ...
        ret += [
            ...
            'CFLAGS=-g -O3 {}'.format(
                wayround_i2p.utils.pkgconfig.pkgconfig('ncurses', '--cflags')
                )
            ]
    ...
    def builder_action_build_define_opts(self, called_as, log):
        ...
        ret += [
            'EXTRA_CFLAGS_XEN_TOOLS={}'.format(
                wayround_i2p.utils.pkgconfig.pkgconfig_include('ncurses')
                ),
            ...
            #    ),
            'CURSES_LIBS={}'.format(
                wayround_i2p.utils.pkgconfig.pkgconfig('ncurses', '--libs')
                ),
            ]

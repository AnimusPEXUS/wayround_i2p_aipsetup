...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def patch_test_c(self):
        name = wayround_i2p.utils.path.join(self.get_src_dir(), 'pam_cap', 'test.c')
    ...
    def patch_pam_cap_c(self):
        name = wayround_i2p.utils.path.join(self.get_src_dir(), 'pam_cap', 'pam_cap.c')
    ...
    def builder_action_after_distribute(self, called_as, log):
        ...
        if host == 'x86_64-pc-linux-gnu' and host == arch:
            ...
            sd = wayround_i2p.utils.path.join(
                self.calculate_dst_install_prefix(),
                'lib',
                'security'
                )

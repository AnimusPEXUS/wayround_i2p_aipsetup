...
class Builder(wayround_i2p.aipsetup.builder_scripts.std.Builder):
    ...
    def builder_action_fix_exim_install(self, called_as, log):
        ...
        exim_install = wayround_i2p.utils.path.join(
            self.get_src_dir(),
            'scripts',
            'exim_install'
            )
    ...
    def builder_action_config_exim(self, called_as, log):
        ...
        editme = wayround_i2p.utils.path.join(
            self.get_src_dir(),
            'src',
            'EDITME'
            )
        editme_makefile = wayround_i2p.utils.path.join(
            self.get_src_dir(),
            'Local',
            'Makefile'
            )
    ...
    def builder_action_config_eximon(self, called_as, log):
        editme_mon = wayround_i2p.utils.path.join(
            self.get_src_dir(),
            'exim_monitor',
            'EDITME'
            )
    ...
    def builder_action_sendmail_link(self, called_as, log):
        lnk = wayround_i2p.utils.path.join(

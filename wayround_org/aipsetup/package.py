...
class ASPackage:
    ...
    def __init__(self, asp_filename):
        ...
        self.filename = wayround_org.utils.path.abspath(asp_filename)
    ...
    def check_package_aipsetup2(self):
        ...
        if not self.filename.endswith('.tar.xz'):
            ...
            if (not os.path.isfile(self.filename)
                ...
                sha512 = wayround_org.utils.checksum.make_file_checksum(
                    self.filename, 'sha512'
                    )

                md5 = wayround_org.utils.checksum.make_file_checksum(
                    self.filename, 'md5'
                    )

                sha512s = \
                    wayround_org.utils.checksum.parse_checksums_file_text(
                        filename_sha512
                        )

                md5s = wayround_org.utils.checksum.parse_checksums_file_text(
                    filename_md5
                    )

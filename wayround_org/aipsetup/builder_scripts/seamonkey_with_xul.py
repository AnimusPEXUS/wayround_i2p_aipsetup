...
def main(buildingsite, action=None):
    ...
    if not isinstance(r, tuple):
        ...
        if 'reduce_destdir_share' in actions and ret == 0:
            idl_path = wayround_org.utils.path.join(
            ...
            if not sm_dir or not xul_dir:
                ...
                idl_sm_path = wayround_org.utils.path.join(idl_path, sm_dir)
                idl_xul_path = wayround_org.utils.path.join(idl_path, xul_dir)

                wayround_org.utils.file.checksumed_dir_redue(
                    idl_sm_path, idl_xul_path
                    )

        if 'reduce_destdir_include' in actions and ret == 0:
            include_path = wayround_org.utils.path.join(
                dst_dir, 'usr', 'include'
                )
            ...
            if not sm_dir or not xul_dir:
                ...
                include_sm_path = wayround_org.utils.path.join(
                    include_path,
                    sm_dir
                    )
                include_xul_path = wayround_org.utils.path.join(
                    include_path,
                    xul_dir
                    )

...
#                    '--out-path={}'.format(dst_dir)
        ...
        if 'build' in actions and ret == 0:
            os.chmod(wayround_org.utils.path.join(src_dir, 'env.sh'), 0o755)
            p = subprocess.Popen(
                ['bash',
                 '-c',
            ...
            p = subprocess.Popen(
                ['bash',
                 ...
                 '. ./env.sh ; kmk install VBOX_PATH_APP_PRIVATE=/usr/lib/virtualbox'.format(
                    wayround_org.utils.path.relpath(dst_dir, src_dir)
                    )
                 ],
                cwd=src_dir
                )
        ...
        if 'copydist' in actions and ret == 0:
            ...
            p = wayround_org.utils.path.join(work_dir, 'out')
            if os.path.isdir(p):
                work_dir = p
            ...
            for i in dirs:
                ...
                work_dir = wayround_org.utils.path.join(
                    p, i, 'release', 'dist'
                    )
